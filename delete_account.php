...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Vérifier si le compte a des transactions associées
        $stmt = Database::getInstance()->query("SELECT COUNT(*) as count FROM Transactions WHERE account_id = :id", [':id' => $id]);
        $count = $stmt->fetch()['count'];

        if ($count > 0) {
            $error = 'Impossible de supprimer ce compte car il a des transactions associées';
        } else {
            $stmt = Database::getInstance()->query("DELETE FROM Accounts WHERE id = :id", [':id' => $id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($success) {
        header('Location: list_accounts.php?message=' . urlencode($success));
    } else {
        header('Location: list_accounts.php?error=' . urlencode($error));
    }
    exit();
}
?>
...
<head>
    ...
    <title>Supprimer un compte — Confiance</title>
    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        ...
        <main class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg shadow-lg p-6">
                <h1 class="text-2xl font-bold text-gray-900 mb-6">Supprimer un compte</h1>
                                ...
                                <div class="mt-4">
                                    <form method="POST" action="">
                                        ...
                                        <div class="flex space-x-3">
                                            ...
                                            <a href="list_accounts.php" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400 transition-colors font-semibold">
                                                Annuler
                                            </a>
                                        </div>
                                    </form>
